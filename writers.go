...
type PackageWriter interface {
	SerializeBranch(branch *Branch, w *bufio.Writer, depth int) error
}

type FigtreeWriter struct{}
...
//
// May return an os.Create package error if the filename could not be created.
func (branch *Branch) WriteToFile(pw PackageWriter, outFilename string) error {
	...
	err = pw.SerializeBranch(branch, writer, 0)
	if eh.Invalid(err) {
		return err
	}
	return writer.Flush()
...
func (branch *Branch) WriteToBuffer(pw PackageWriter) (string, error) {
...
//
// May return any of the errors generated by bufio.Writer.
func (fw FigtreeWriter) SerializeBranch(branch *Branch, w *bufio.Writer, depth int) error {
	...
	for _, item := range branch.Items {
		...
		case *Branch:
			...
			_ = fw.SerializeBranch(value, w, depth+1)
...
//
// May return any of the errors generated by bufio.Writer.
func (iw InternalWriter) SerializeBranch(branch *Branch, w *bufio.Writer, depth int) error {
	prefix := strings.Repeat("\t", depth)
	var err error
	...
	for _, item := range branch.Items {
		...
		case *Branch:
			...
			_ = iw.SerializeBranch(value, w, depth+1)
			_, err = fmt.Fprintf(w, "%s}\n", srcContext)
			if err != nil {
				return err
			}
...
//
// May return any of the errors generated by bufio.Writer.
func (jw JsonWriter) SerializeBranch(branch *Branch, w *bufio.Writer, depth int) error {
	fmt.Fprintf(w, "{")
	err := jw.serializeBranch(branch, w, depth+1)
	if err != nil {
		return err
	}
	fmt.Fprintf(w, "\n}")
...
func (jw JsonWriter) serializeBranch(branch *Branch, w *bufio.Writer, depth int) error {
	...
	for _, item := range branch.Items {
		...
		if bIsArray || len(allItems) > 1 {
			...
			err = jw.serializeArray(key, allItems, branch, w, depth)
			if err != nil {
				return err
			}
			commaLF = ",\n"
		...
		case *Branch:
			...
			_ = jw.serializeBranch(value, w, depth+1)
			_, err = fmt.Fprintf(w, "\n%s}", prefix)
			if err != nil {
				return err
			}
...
func (jw JsonWriter) serializeArray(keyName string, allItems []Item, branch *Branch, w *bufio.Writer, depth int) error {
	...
	for _, item := range allItems {
		...
		case *Branch:
			...
			_ = jw.serializeBranch(value, w, depth+2)
			_, err = fmt.Fprintf(w, "\n%s}", prefix1)
			if err != nil {
				return err
			}
...
//
// May return any of the errors generated by bufio.Writer.
func (yw YamlWriter) SerializeBranch(branch *Branch, w *bufio.Writer, depth int) error {
	fmt.Fprintf(w, "---\n")
	err := yw.serializeBranch(branch, w, depth)
	if err != nil {
		return err
	}
	fmt.Fprintf(w, "\n")
...
func (yw YamlWriter) serializeBranch(branch *Branch, w *bufio.Writer, depth int) error {
	...
	for _, item := range branch.Items {
		...
		allItems := branch.QueryAll(key)
		if bIsArray || len(allItems) > 1 {
			err = yw.serializeArray(key, allItems, branch, w, depth)
			if err != nil {
				return err
			}
			arrayKeys[key] = true
		...
		case *Branch:
			...
			_ = yw.serializeBranch(value, w, depth+1)
			if err != nil {
				return err
			}
		}
...
// keyName may end in trailing brackets []
// allItems may be a collection of 0, 1 or more items
func (yw YamlWriter) serializeArray(keyName string, allItems []Item, branch *Branch, w *bufio.Writer, depth int) error {
	...
	for _, item := range allItems {
		...
		case *Branch:
			fmt.Fprintf(w, "%s-\n", prefix1)
			_ = yw.serializeBranch(value, w, depth+2)
		}
	}
